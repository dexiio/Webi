...
public class SessionArrayValue extends ArrayValueWrapper
    ...
    /**
     * Changes the session id. Used by session_regenerate_id() where we want to change the session id, but keep the rest
     ...
     *
     * <p>If the session data has changed and we have persistent sessions, save the session. However, if
     * save-on-shutdown is true, only save on a server shutdown.
     */
    public void finish() {
        int count;
    ...
    /*
     * public void setClusterObject(ClusterObject clusterObject) { _clusterObject = clusterObject; }
     */
    ...
    public void removeEvent() {
         ...
         *
         * if (log.isLoggable(Level.FINE)) { log.fine("remove session " + _id); }
         *
         * long now = Alarm.getCurrentTime();
         *
         * ClusterObject clusterObject = _clusterObject;
         *
         * if (_isValid && clusterObject != null) { try { clusterObject.update(); clusterObject.store(this); } catch
         * (Throwable e) { log.log(Level.WARNING, "Can't serialize session", e); } }
         */
    }

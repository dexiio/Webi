...
public class SessionModule extends AbstractQuercusModule
    ...
    /**
     * Returns and/or sets the value of session.cache_limiter, affecting the cache related headers that are sent as a
     * result of a call to
     * {@link #session_start(Env)}.
     *
     * If the optional parameter is not supplied, this function simply returns the existing value. If the optional
     * parameter is supplied, the returned value is the old value that was set before the new value is applied.
     *
     * Valid values are "nocache" (the default), "private", "private_no_expire", and "public". If a value other than
     * these values is supplied, then a warning is produced and no cache related headers will be sent to the client.
     */
     ...
     *
     * This function first creates a new session id. The old session values are migrated to this new session. Then a new
     * session cookie is sent (XXX: send only if URL rewriting is off?). Changing the session ID should be transparent.
